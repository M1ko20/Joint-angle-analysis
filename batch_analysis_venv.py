#!/usr/bin/env python3
"""
Batch analýza VENV pose detection modelů na všech videích
Spouští pouze venv modely (MediaPipe, MoveNet, YOLO, ViTPose, MediaPipe3D)
Pro conda modely použij batch_analysis_conda.py
"""
...
try:
    ...
    print(f"⚠️ Venv detektory nedostupné: {e}")
    sys.exit(1)


class BatchAnalyzerVenv:
    """Batch analýza VENV modelů na všech videích"""
    
    def __init__(self, videos_root="video", output_root="outputvenv", confidence_threshold=0.5):
        ...
        print(f"\n{'='*80}")
        print(f"🎯 BATCH POSE ANALYSIS - VENV MODELY")
        print(f"{'='*80}")
        print(f"📁 Videa: {self.videos_root}")
        print(f"📁 Output: {self.output_root}")
        print(f"🎚️ Confidence: {self.confidence_threshold}")
        print(f"🔧 Dostupné modely: {len(self.venv_models)}")
        for model in self.venv_models:
            print(f"   • {model}")
        print(f"{'='*80}\n")
    ...
    def run_all(self):
        """Spustí analýzu všech venv modelů na všech videích"""
        ...
        total = len(self.venv_models) * len(videos)
        current = 0
    ...
    def _save_summary(self, results):
        ...
        summary_file = self.output_root / "batch_summary_venv.json"
    ...
    def _print_summary(self, results):
        """Vypíše souhrn"""
        print(f"\n{'='*80}")
        print(f"📊 SOUHRN BATCH ANALÝZY - VENV")
        print(f"{'='*80}\n")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Batch analýza VENV pose detection modelů")
    parser.add_argument("--videos", "-v", type=str, default="video",
                       help="Cesta k složce s videi (default: video)")
    parser.add_argument("--output", "-o", type=str, default="outputvenv",
                       help="Výstupní složka (default: outputvenv)")
    ...
    if args.model:
        analyzer.venv_models = [m for m in analyzer.venv_models if m == args.model]
        if not analyzer.venv_models:
