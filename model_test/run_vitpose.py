"""
===== run_vitpose.py =====
ViTPose Runner - 2 varianty: Large/Huge (pouze video)
"""
# ULOŽ JAKO: run_vitpose.py
...
def process_vitpose(video_path, output_dir, model="vitpose_large", confidence=0.5):
    detector_name = model
...
def main():
    ...
    results = {}
    for model in ["vitpose_large", "vitpose_huge"]:
        output_dir = os.path.join(args.output_base, model)
        results[model] = process_vitpose(args.video, output_dir, model, args.confidence)
    
    print(f"\n{'='*80}\n📊 VITPOSE SOUHRN\n{'='*80}\n")
    for name, success in results.items():
        print(f"{'✅' if success else '❌'} {name}")
    print(f"\n✅ Úspěšných: {sum(1 for v in results.values() if v)}/2")
